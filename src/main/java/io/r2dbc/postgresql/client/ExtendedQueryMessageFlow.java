    ...
    public static Flux<BackendMessage> execute(Binding binding, Client client, PortalNameSupplier portalNameSupplier, String statementName, String query, boolean forceBinary,
        ...
        if (fetchSize == NO_LIMIT) {
            return Flux.defer(() -> {
                ...
                Flux<FrontendMessage> bindFlow = toBindFlow(binding, portal, statementName, query, forceBinary, fetchSize).concatWithValues(new Close(portal, PORTAL))
                    .concatWithValues(Sync.INSTANCE);
